...
namespace Padguide_Scrape
{
    class DB_scrape
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<List<String>> MonsID = CSV_Parser(path + "\\TBL_MONSTER.csv"); //Remove commas and quotes, sort by MONSTER_NO_US
            ...
            while (i < ValidOpenTime.Count)
            {
                ...
                while (j < Difficulty.Count)
                {
                    ...
                    if (test.ToString().Contains(Decrypt(Difficulty[j][5], "<Replace with AESKEY>") + "::: "))
                    {
                        j++;
                        continue;
                    }
                    test.Append(Decrypt(Difficulty[j][5], "<Replace with AESKEY>") + "::: ");
            ...
            req.Headers.Add("Authorization: token <gist token auth>");
