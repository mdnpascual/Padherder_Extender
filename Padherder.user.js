...
(function() {
    ...
    var commitVer = GM.getValue("commitVer", -1);
    var dict = GM.getValue("OffsetSeekerCacheStore", {});
    ...
    fn.push($.get("https://gist.githubusercontent.com/padxExtender/e11935cf82505acbdd2b6e03a8cfe440/raw/file1.txt", function( database_scraped ) {
        lines = database_scraped.split("\n");
        storedday = lines[0];
    }));//2 links because gist is funky with updating hard links
    fn.push($.get("https://gist.githubusercontent.com/padxExtender/e11935cf82505acbdd2b6e03a8cfe440/raw/", function( database_scraped ) {
        lines2 = database_scraped.split("\n");
        storedday2 = lines2[0];
    }));
    //Get api data
    fn.push($.get("https://www.padherder.com/user-api/user/" + username.textContent + "/", function( padx_scraped ) {
        data_user = padx_scraped;
    }));
    fn.push($.get("https://raw.githubusercontent.com/mdnpascual/Padherder_Extender/master/json_template/evolutions.json", function( padx_scraped ) {
        data_evo = JSON.parse(padx_scraped);
        data_evo_string = padx_scraped;
    }));
    fn.push($.get("https://raw.githubusercontent.com/mdnpascual/Padherder_Extender/master/json_template/monsters.json", function( padx_scraped ) {
        mons_data = JSON.parse(padx_scraped);
        mons_data_string = padx_scraped;
    }));
    fn.push($.get("https://raw.githubusercontent.com/mdnpascual/Padherder_Extender/master/json_template/active_skills.json", function( padx_scraped ) {
        active_skill = JSON.parse(padx_scraped);
        active_skill_string = padx_scraped;
    }));
    fn.push($.get("https://api.github.com/repos/mdnpascual/Padherder_Extender/commits", function( github_scrape ) {
        commitVer.then(function(result) {
            if (result != github_scrape[0].sha){
                dict = {};
                GM.setValue("commitVer", github_scrape[0].sha);
            }
            else{
    ...
    $.when(fn[0], fn[1], fn[2], fn[3], fn[4], fn[5], fn[6]).done(function() {
        ...
        while (i < mats_format.length){
            ...
            //Dungeonsq
            stringappend += uniq(mats_format[i][2].split(":::")).join("<br>").split("|||").join("<br>") + '</span><img src="http://pad.dnt7.com/';
            //img_url
        ...
        while (i < filter_mons_need_skillup.length){
            ...
            if (j > splitting.length - 2) //reset
                j = 1;
            stringappend += '</span><img src="http://pad.dnt7.com/';
            //img_url
        ...
        GM.setValue("OffsetSeekerCacheStore", dict);
    });
